...
export default function DashboardLayout({ role = "client" }: { role?: "client" | "admin" }) {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside className="w-64 bg-black/40 border-r border-white/10 hidden md:flex flex-col fixed h-full backdrop-blur-xl z-20">
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-h-screen">
        {/* Top Navbar */}
        <header className="h-16 border-b border-white/10 bg-background/50 backdrop-blur-md sticky top-0 z-10 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-white/10">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-medium text-white">{role === 'admin' ? 'Admin User' : 'Alex Johnson'}</p>
                <p className="text-xs text-gray-400">{role === 'admin' ? 'Administrator' : 'Premium Plan'}</p>
              </div>
