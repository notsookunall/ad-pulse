...
export function generateLocalCampaignAnalysis(campaigns: Campaign[]): CampaignAnalysis {
  ...
  return {
    ...
    notice: hasGeminiApiKey()
      ? "Gemini was unavailable for this request, so the built-in recommendation engine generated the fallback insights."
      : "Add VITE_GEMINI_API_KEY in your .env.local file to upgrade these recommendations from rules-based insights to Gemini-generated analysis.",
  };
}
